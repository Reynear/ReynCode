/**
 * GeminiDriver — `ProviderDriver` for Gemini CLI ACP runtime.
 *
...
import { makeGeminiContinuationGroupKey, makeGeminiEnvironment } from "./GeminiHome.ts";
...
export const GeminiDriver: ProviderDriver<GeminiSettings, GeminiDriverEnv> = {
  ...
  create: ({ instanceId, displayName, accentColor, environment, enabled, config }) =>
    Effect.gen(function* () {
      ...
      const processEnv = yield* makeGeminiEnvironment(effectiveConfig, mergedEnvironment).pipe(
      ...
      const continuationGroupKey = yield* makeGeminiContinuationGroupKey(effectiveConfig).pipe(
        Effect.provideService(Path.Path, path),
      );
