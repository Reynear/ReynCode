/**
 * GeminiAdapterLive — Gemini CLI (`gemini --experimental-acp`) via ACP.
 *
...
export interface GeminiAdapterLiveOptions {
   ...
   * Defaults to the legacy built-in instance id (`gemini`).
   */
   ...
   * the `geminiSettings` captured at construction.
   *
...
export function makeGeminiAdapter(
  ...
  return Effect.gen(function* () {
    ...
    const logNative = (
      ...
      method: string,
      payload: unknown,
      _source: "acp.jsonrpc" | "acp.gemini.extension",
    ) =>
      Effect.gen(function* () {
    ...
    const emitPlanUpdate = (
      ...
      rawPayload: unknown,
      source: "acp.jsonrpc" | "acp.gemini.extension",
      method: string,
    ) =>
      Effect.gen(function* () {
    ...
    const startSession: GeminiAdapterShape["startSession"] = (input) =>
      withThreadLock(
        ...
        Effect.gen(function* () {
          ...
          // Resolve the GeminiSettings used to spawn the ACP child. Production
          ...
          // `updateSettings({ providers: { gemini: { binaryPath } } })` calls
    ...
    const sendTurn: GeminiAdapterShape["sendTurn"] = (input) =>
      Effect.gen(function* () {
        ...
        const resolvedModel = resolveGeminiAcpModelId(model);
        yield* applyRequestedSessionConfiguration({
          runtime: ctx.acp,
